...
pub struct Ps2Frame {
    /// Which wire variant the framer interpreted this frame as. Drives
    /// the classifier (see [`crate::classifier`]).
    pub kind: FrameKind,
    pub data: u8,
    pub parity_ok: bool,
    pub framing_ok: bool,
...
/// How the framer interpreted the bit stream that produced this frame.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[cfg_attr(feature = "defmt", derive(defmt::Format))]
pub enum FrameKind {
    /// 11-bit AT / PS-2 frame: start(0) + 8 data + parity + stop(1).
    /// Emitted when the bit counter reaches 11.
    At,
    /// 9-bit XT frame: start(1) + 8 data. Emitted on the inter-frame
    /// idle gap (XT has no stop bit to anchor on).
    Xt,
    /// Partial frame timed out without reaching either valid length.
    Invalid,
}
...
impl Framer {
    ...
    fn build_frame(&self, total_bits: u8) -> Ps2Frame {
        ...
        let (kind, parity_ok, framing_ok) = if total_bits == AT_BIT_COUNT {
            ...
            (FrameKind::At, parity_ok, framing_ok)
        } else {
            let start_bit = (self.assembled & 1) as u8;
            (FrameKind::Xt, true, start_bit == 1)
        };
